...
import { lib as cryptoNode } from '../src/crypto/crypto-node.js'
...
describe('Noise', () => {
  ...
  it('should test that secureOutbound is spec compliant', async () => {
    ...
    const [outbound, { wrapped, handshake }] = await Promise.all([
      ...
      (async () => {
        ...
        const staticKeys = cryptoNode.generateX25519KeyPair()
        const xx = new XX(cryptoNode)

        const payload = await getPayload(remotePeer, staticKeys.publicKey)
        const handshake = new XXHandshake(false, payload, prologue, cryptoNode, staticKeys, wrapped, localPeer, xx)
  ...
  it('should accept a prologue', async () => {
    try {
      const noiseInit = new Noise({ staticNoiseKey: undefined, crypto: cryptoNode, prologueBytes: Buffer.from('Some prologue') })
      const noiseResp = new Noise({ staticNoiseKey: undefined, crypto: cryptoNode, prologueBytes: Buffer.from('Some prologue') })
